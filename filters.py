    ...
    with sb:
        ...
        if sidebar:
            ...
            if st.button(label="Create URL for Filters", 
                     help=help_msg):
                print(f"Outputing query: {st.session_state['filters']}")
        ...
        data = time_periods
        with col1:
            selection[filter] = st.selectbox("Time Range (for tables and bar charts only)", data, index=_get_index(filter, data, 1),
